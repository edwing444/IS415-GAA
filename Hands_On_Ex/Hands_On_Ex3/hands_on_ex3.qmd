...
['Spatial']{.underline} classes are specific classes provided by the 'sp' package to represent different types of spatial data. These classes include:

1.  **SpatialPoints**: Represents spatial data as a set of points. Each point is defined by coordinates in a spatial reference system.

2.  **SpatialLines**: Represents spatial data as a set of lines, where each line is composed of a series of connected points.

3.  **SpatialPolygons**: Represents spatial data as a set of polygons, which are defined by a closed sequence of points. Polygons are used to represent areas.

A [generic 'sp' object]{.underline} typically refers to spatial objects created and handled using the 'sp' package.
...
[Structure]{.underline}: 'Spatial' classes have a specific structure and are designed to handle different types of spatial data like points, lines, polygons etc. Whereas, Generic 'sp' objects refer to any object created using the 'sp' package, which could be any of these specific 'Spatial' classes or others.

[Methods:]{.underline} Different 'Spatial' classes have methods specifically designed for the type of data they represent. For example, methods for 'SpatialPolygons'would include operations relevant to polygons, such as checking if a point is inside a polygon.
